export const loadDOM = function (listOfJobs) {
  ...
  export const workCard = function(job) {
    ...
    <div class="md:w-1/3 mr-6 flex-none">
      <h4><a href="https://aws.amazon.com/" target="_blank" rel="noreferrer">${job.companyName}</a></h4>
      <p class="m-0 mt-1 leading-normal text-base text-white opacity-50">${job.jobTitle}</p>
      <p class="m-0 mt-1 leading-normal text-base text-white opacity-50">${job.dates}</p>
    </div>
    <div class="mt-2 md:mt-0 text-purpleLight">
      <p class="m-0 leading-normal"> ${job.bullet1}</p>
    </div>
  </div>
